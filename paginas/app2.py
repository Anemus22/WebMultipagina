...
#Contenido de pagina
st.title("Curso de Programación en Java")
...
if opcion == "Introducción":
    st.markdown("Bienvenido al curso básico de programación en Java.")
    st.markdown("Este curso tiene niveles y diferentes recursos que te perimitiran saber lo basico de c++")
    st.video("https://www.youtube.com/watch?v=i4OWlnyAkzc")

elif opcion == "Ir al curso":
    st.info("Haz clic abajo para abrir el curso completo.")
    st.markdown(f"[Abrir Curso]({backend_url})")
    lottie_java = load_lottiefile("./animaciones/Animation6.json")
    st_lottie(
    lottie_java,
    speed=2,
    reverse=False,
...
st.warning("⚠️ Importante ejecutar flask en una terminal con el codigo = python Java.py desde la carpte Flask")